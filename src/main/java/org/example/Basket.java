...
public class Basket {
    ...
    public void addToCart(int productNum, int amount) {
        if ((productNum + 1) > products.length || (productNum + 1) <= 0) {
            throw new ArrayIndexOutOfBoundsException("Номер товара от 1 до " + products.length);
        } else {
            if (amount <= 0) {
                throw new IllegalArgumentException("Количество товара не может быть отрицательным и должно быть больше 0!");
            } else {
